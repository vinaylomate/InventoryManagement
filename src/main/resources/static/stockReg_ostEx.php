...
set_time_limit(0);
$filename="oststockReg_Ex_".date('d.m.Y H:i:s a').".xls";
$date=date('Y-m-d');
...
WHERE a.sts!='2' AND IF(a.iType='1',r.sts!='2',f.sts!='2') AND a.stk_qty='0' ".$str." ORDER BY a.ID";
		//echo "A : ".$qq."<br>";
	}
	else
	{
...
WHERE a.sts!='2' AND IF(a.iType='1',r.sts!='2',f.sts!='2') AND a.stk_qty='0' ".$str." AND u.uid='$uId' ORDER BY a.ID";
		//echo "B : ".$qq."<br>";
	}
}
else
...
LEFT JOIN category_master AS rg ON r.catId=rg.catId 
LEFT JOIN category_master AS fgg ON f.category_id=fgg.catId 
WHERE a.sts!='2' AND IF(a.iType='1',r.sts!='2',f.sts!='2') AND a.stk_qty='0' ORDER BY a.ID";
	}
	else
	{
		if($uRole=='admin' && ($uiType=='3' || $uiType=='0'))
