...
function getdata()
{
	...
	var uRole=document.getElementById('urole').value;
	var uloc= document.getElementById('uloc').value;
	//var rep_tp=document.getElementById('rep_tp').value;
 	//var scriptUrl='getfastMovingItem.php?compId='+compId+'&iType='+iType+'&locId='+locId+'&uId='+uId+'&uiType='+uType+'&role='+uRole+'&srch='+srch+'&uloc='+uloc;+'&rep_tp='+rep_tp+'&fdt='+fdt+'&tdt='+tdt;
 	var scriptUrl='getfastMovingItem.php?compId='+compId+'&iType='+iType+'&locId='+locId+'&uId='+uId+'&uiType='+uType+'&role='+uRole+'&srch='+srch+'&uloc='+uloc+'&fdt='+fdt+'&tdt='+tdt;
	//alert(scriptUrl);
 	//dataTable
 	$('#tbl').load(scriptUrl);
}
...
function dataReset()
{
	window.location.href='fastMovingItemRpt.php';
}
...
function getexcel()
{
	...
	var uRole=document.getElementById('urole').value;
	var uloc= document.getElementById('uloc').value;
 	var scriptUrl='fastMovingItemEx.php?compId='+compId+'&iType='+iType+'&locId='+locId+'&uId='+uId+'&uiType='+uType+'&role='+uRole+'&srch='+srch+'&uloc='+uloc+'&fdt='+fdt+'&tdt='+tdt;
...
function getpdf()
{
	...
	var uloc= document.getElementById('uloc').value;
 	...
 	var scriptUrl='mpdf/print_files/fastMovingItem_pdf.php?compId='+compId+'&iType='+iType+'&locId='+locId+'&uId='+uId+'&uiType='+uType+'&role='+uRole+'&srch='+srch+'&uloc='+uloc+'&fdt='+fdt+'&tdt='+tdt;
	//alert(scriptUrl);
 	//dataTable
 	window.open(scriptUrl,  "toolbar=yes, scrollbars=yes, resizable=no, top=60, left=0, width=1000, height=600");
}
...
<h1 class="h3 mb-0 text-gray-801" style="padding-bottom: 5px;">Fast Moving Item Report</h1>   
</div>
...
<!--<td valign="top"><label class="m-0 font-weight-bold text-primary">Type</label></td>	
<td valign="top">:</td>	
<td valign="top">
<select class="chosen-select form-control form-control-sm" id="rep_tp" name="rep_tp" onChange="getdata();" tabindex="3" style="width:230px;" >
...
LEFT JOIN `inventory_info_tbl` AS b ON a.ID=b.invId
 ...
 WHERE a.sts!='2' AND IF(a.iType='1',r.sts!='2',f.sts!='2') AND b.qty!='0' AND b.eType='2'
GROUP BY IF(a.iType='1',r.focus_code,f.focus_code), IF(a.iType='1',r.sage_code,f.sage_code), IF(a.iType='1',CONCAT(rg.category_nm,' - ',r.description,' - ',ru.uom),CONCAT(fgg.category_nm,' - ',f.brand,' ',f.descc,' - ',fu.uom)), CONCAT(l.loc_code,' - ',l.location_name) ,a.iType 
...
LEFT JOIN `inventory_info_tbl` AS b ON a.ID=b.invId
 ...
 LEFT JOIN category_master AS rg ON r.catId=rg.catId 
 LEFT JOIN category_master AS fgg ON f.category_id=fgg.catId
  WHERE a.sts!='2' AND IF(a.iType='1',r.sts!='2',f.sts!='2') AND b.eType='2' AND a.compId='$cmpId' AND a.uid='$userId' AND b.qty!='0' 
  GROUP BY IF(a.iType='1',r.focus_code,f.focus_code), IF(a.iType='1',r.sage_code,f.sage_code), IF(a.iType='1',CONCAT(rg.category_nm,' - ',r.description,' - ',ru.uom),CONCAT(fgg.category_nm,' - ',f.brand,' ',f.descc,' - ',fu.uom)), CONCAT(l.loc_code,' - ',l.location_name) ,a.iType 
  ORDER BY SUM(b.qty) DESC";
