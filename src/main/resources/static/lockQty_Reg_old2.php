...
{
	$qq="SELECT a.ID,a.dt,a.compId,a.locId,a.iType,a.docNo,a.yr,a.yr2,b.company_code,b.company_name,c.loc_code,c.location_name,IF(a.iType='1','RAW MATERIAL','FINISHED GOODS') AS iTypeNm,ref,notes ,a.salemnId,s.salemanNm FROM inventory_tbl AS a LEFT JOIN company_tbl AS b ON a.compId=b.ID LEFT JOIN location_tbl AS c ON a.locId=c.ID LEFT JOIN salesman_tbl AS s ON a.salemnId=s.ID WHERE a.sts!='2' AND b.sts!='2' AND c.sts!='2' AND a.lsts='1' AND s.sts!='2' AND a.compId='$cmpId' AND a.iType='$uiType' AND a.uid='$userId' AND a.lsts='1' ORDER BY substring(a.docNo,1,9), substring(a.docNo,10)*1 DESC";
	//echo "Else  - ".$qq."<Br>";
