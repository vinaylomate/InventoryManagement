...
for($i=0;$i<$c;$i++)
{
 /* echo $invId[$i]." : ".$itemId[$i]." : ".$qty[$i]." : ".$batchno[$i]." : ".$expdt[$i]." : ".$rordqty[$i]." : ".$rackNo[$i]." : ".$stkIds[$i]." : ".$bstkId[$i]." : ".$bentryId[$i]."<Br>";*/	
  if($eType=='1')
  {
	  $qstk01="SELECT expdt FROM stock_tbl_2 WHERE ID='$bstkId[$i]'";
      //echo "IN - AA : ".$qstk01."<Br>";	
      $stmtstk01=$mysql->prepare($qstk01);
      $stmtstk01->execute();	
	  $rowexp01=$stmtstk01->fetch(PDO::FETCH_ASSOC);
	  $expdt01=$rowexp01['expdt'];
  }
  else
  {
	  $expdt01=$expdt[$i]; 
  }
  $qinvInfo="INSERT INTO inventory_info_tbl(invId,itemId,entryId,eType,qty,batchNo,expdt,lockinfoId)	VALUES('$invIds','$itemId[$i]','$eTypeId','$eType','$qty[$i]','$batchno[$i]','$expdt01','$invId[$i]')";
  //echo $qinvInfo."<br>";	
  ...
  if($eType=='1')
  {
    ...
    else
    {
	  ...
	  $qstk21="SELECT expdt,stk_qty FROM stock_tbl_2 WHERE ID='$bstkId[$i]'";
      //echo "IN - AA : ".$qstk21."<Br>";	
      $stmtstk21=$mysql->prepare($qstk21);
      $stmtstk21->execute();	
	  $rowexp=$stmtstk21->fetch(PDO::FETCH_ASSOC);
	  $expdt1=$rowexp['expdt'];
	  $stkql2=$rowexp['stk_qty'];
	  $stkqbb=$stkql2+$qty[$i];
	  //echo $stkql2. " : ".$stkqbb."<Br>";
		
	  $qstk23="UPDATE stock_tbl_2 SET stk_qty='$stkqbb' WHERE ID='$bstkId[$i]' AND sts!='2'";
	  //echo "IN - AA : ".$qstk23."<Br>";
...
$mysql=null;
echo "Record Released Successfully...!"."||".$invIds;
?>