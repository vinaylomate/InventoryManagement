...
set_time_limit(0);
$filename="nonMovingItemEx".date('d.m.Y H:i:s a').".xls";
$date=date('Y-m-d');
...
{	
	...
	{
		$qq="SELECT a.ID,a.compId,a.locId,l.loc_code,l.location_name AS locNm,a.iType,a.itemId,SUM(b.in_qty+b.openstk) AS in_qty,a.stk_qty AS stk_qty FROM stock_tbl AS a LEFT JOIN stock_tbl_2 AS b ON a.ID=b.stkId LEFT JOIN location_tbl AS l ON a.locId=l.ID WHERE a.in_qty=a.stk_qty AND (a.out_qty='0' OR a.lock_qty='0') ".$str." GROUP BY a.ID,a.compId,a.locId,a.iType,a.itemId ORDER BY a.stk_qty DESC";
		//echo "A : ".$qq."<br>";
	}
	else
	{
		$qq="SELECT a.ID,a.compId,a.locId,l.loc_code,l.location_name AS locNm,a.iType,a.itemId,SUM(b.in_qty+b.openstk) AS in_qty,a.stk_qty AS stk_qty FROM stock_tbl AS a LEFT JOIN stock_tbl_2 AS b ON a.ID=b.stkId LEFT JOIN location_tbl AS l ON a.locId=l.ID LEFT JOIN admin_usr_loc AS u ON l.ID=u.locId WHERE u.uid='$uId' AND a.in_qty=a.stk_qty AND (a.out_qty='0' OR a.lock_qty='0') ".$str." GROUP BY a.ID,a.compId,a.locId,a.iType,a.itemId ORDER BY a.stk_qty DESC";
		//echo "B : ".$qq."<br>";
	}
	//echo "B : ".$qq."<br>";
...
{	
	...
	if($compId=='0')
	{
		$qq="SELECT a.ID,a.compId,a.locId,l.loc_code,l.location_name AS locNm,a.iType,a.itemId,SUM(b.in_qty+b.openstk) AS in_qty,a.stk_qty AS stk_qty FROM stock_tbl AS a LEFT JOIN stock_tbl_2 AS b ON a.ID=b.stkId LEFT JOIN location_tbl AS l ON a.locId=l.ID WHERE a.in_qty=a.stk_qty AND (a.out_qty='0' OR a.lock_qty='0') GROUP BY a.ID,a.compId,a.locId,a.iType,a.itemId ORDER BY a.stk_qty DESC";
	  //echo "AAC : ".$qq."<br>";
	}
	else
	{
		...
		{
			$qq="SELECT a.ID,a.compId,a.locId,l.loc_code,l.location_name AS locNm,a.iType,a.itemId,SUM(b.in_qty+b.openstk) AS in_qty,a.stk_qty AS stk_qty FROM stock_tbl AS a LEFT JOIN stock_tbl_2 AS b ON a.ID=b.stkId LEFT JOIN location_tbl AS l ON a.locId=l.ID LEFT JOIN admin_usr_loc AS u ON l.ID=u.locId WHERE u.uid='$userId' AND a.in_qty=a.stk_qty AND (a.out_qty='0' OR a.lock_qty='0') ".$str." GROUP BY a.ID,a.compId,a.locId,a.iType,a.itemId ORDER BY a.stk_qty DESC";
	  //echo "AAB : ".$qq."<br>";
		}
	}
	//echo "hhh : ".$qq."<br>";
...
foreach($paginationData as $row)
{
	...
	if($row['iType']=='1')
	{
		...
		if(!empty($rowi['fc']))
		{
			...
			echo '<td style="background-color:#0d69f445;color:#000">RAW MATERIAL</td>';
	...
	else
	{
		...
		{
			echo'<tr style="color:#000;font-weight:600;">';
			echo '<td style="background-color:#f4b80d61;color:#000">FINISHED GOODS</td>';
...
<th style="color:#fff;" align="center"></th>
<th style="color:#fff" align="center"></th>
<th style="color:#fff;align="right">Total</th>
<th style="color:#fff;align="right">'.$a.'</th>
</tr>
</tfoot>';
echo '</table>';
$mysql=null;
