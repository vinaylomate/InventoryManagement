...
function getLoc()	
{
  ...
  else	
  {
	  ...
	  var scriptUrl ='getLocations.php?compId='+id+'&iType='+iType;
	  //alert(scriptUrl);
...
function getdata()
{
 	...
 	else
    {
 	...
 	var scriptUrl='getStockRegLoc2.php?compId='+compId+'&iType='+iType;
	//alert(scriptUrl);
 	//dataTable
 	$('#tbl').load(scriptUrl);
}
...
function getpdf()
{
 	...
 	else
    {
 	...
 	var scriptUrl='mpdf/print_files/stklock2_pdf.php?compId='+compId+'&iType='+iType;
	//alert(scriptUrl);
 	//dataTable
                ...
                <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                  <thead>
                    <tr style="background-color:#b02923">
					  ...
					  <?php
					  $ql="SELECT loc_code,location_name FROM location_tbl WHERE sts!='2' AND iType='2' ORDER BY loc_code";	
				  ...
				  while($row=$stm->fetch(PDO::FETCH_ASSOC))
				  {
					  ...
					  $ql="SELECT ID FROM location_tbl WHERE sts!='2' AND iType='2' ORDER BY loc_code";	
					  $stl=$mysql->prepare($ql);
					  $stl->execute();
					  $tot=0;
					  while($row1=$stl->fetch(PDO::FETCH_ASSOC))
					  {
						  $locId=$row1['ID'];
						  $qqq="SELECT SUM(a.stk_qty) AS stk FROM stock_tbl AS a LEFT JOIN company_tbl AS c ON a.compId=c.ID LEFT JOIN location_tbl AS l ON a.locId=l.ID LEFT JOIN rm_master AS r ON a.itemId=r.rmId LEFT JOIN uom_tbl AS ru ON r.UOM=ru.ID LEFT JOIN fg_master AS f ON a.itemId=f.fgId LEFT JOIN uom_tbl AS fu ON f.uom=fu.ID WHERE a.sts!='2' AND IF(a.iType='1',r.sts!='2',f.sts!='2') AND  a.iType='2' AND a.locId='$locId' AND a.itemId='$itemId' ORDER BY a.ID";
