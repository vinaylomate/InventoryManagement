...
<input type="text" class="form-control form-control-sm" id="txtsearch" name="txtsearch" placeholder="Search" tabindex="7.1" value="" onChange="getproduct()">
</div>	
<select class="chosen-select form-control form-control-sm" id="itemId" name="itemId" tabindex="7.1" onChange="getProductInfo();">
<option value="0">Select</option>
</select>
</div>  
...
while($rw=$stmtt->fetch(PDO::FETCH_ASSOC))
{
	...
	$q11="SELECT ID,out_qty FROM stock_tbl_2 WHERE compId='$compId' AND locId='$locId' AND iType='$iTyps' AND itemId='".$rw['itemId']."' AND batchNo='".$rw['batchNo']."' AND sts='0' AND dt='$dt' AND out_qty='".$rw['qty']."'";
	//echo $q11."<br>";
