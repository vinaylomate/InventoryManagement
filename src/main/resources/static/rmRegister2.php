...
.form-control_lbl {
    ...
    border-radius: 0.35fff    transition: border-color .15s ease-in-out, box-shadow .15s ease-in-out;*/
}	
</style>
<script>
function edit(id)
...
function dataReset()
{
	window.location.href='rmRegister2.php';
}
...
<td valign="top"><input type="button" id="btnsearch" name="btnsearch" class="btn btn-primary" onClick="searchFilter();" tabindex="3.2" value="Search"></td>	
<td valign="top"></td>	
<td valign="top">

</td>	
<td>
<!--<input type="button" id="btnsearch" name="btnsearch" class="btn btn-primary" onClick="searchFilter();" tabindex="3.2" value="Search">-->
</td>
</tr>	
</table>	
...
<table>
<tr>
<td valign="top" align="right"><span class="form-control_lbl">Search</span></td>	
<td valign="top" align="right"><span class="form-control_lbl">:</span></td>	
<td valign="top" align="right"><input type="text" id="txtsearch" name="txtsearch" class="form-control" tabindex="3.1" placeholder="Type keywords..." onkeyup="searchFilter();" style="width: 250px;"></td>	
</tr>	
</table>	
</td>	
</tr>	
...
{
	$qq="SELECT a.rmId,a.sage_code,a.focus_code,a.description,b.uom AS unit_name,CONCAT(c.company_code,' - ',c.company_name) AS company_name,CONCAT(d.loc_code,' - ',d.location_name) AS location_name,a.reorder_level_qty,a.product_expiry,a.rackNo,e.category_nm,a.location_id FROM `rm_master` AS a LEFT JOIN `uom_tbl` AS b ON a.UOM=b.ID LEFT JOIN `company_tbl` AS c ON a.company_id=c.ID LEFT JOIN `location_tbl` AS d ON a.location_id=d.ID LEFT JOIN category_master AS e ON a.catId=e.catId WHERE a.sts='0' ORDER BY a.rmId LIMIT $limit";
	//echo $qq."<Br>";
	
	$qqq="SELECT COUNT(a.rmId) AS rowNum FROM `rm_master` AS a LEFT JOIN `uom_tbl` AS b ON a.UOM=b.ID LEFT JOIN `company_tbl` AS c ON a.company_id=c.ID LEFT JOIN `location_tbl` AS d ON a.location_id=d.ID LEFT JOIN category_master AS e ON a.catId=e.catId WHERE a.sts='0' ORDER BY a.rmId";
}
else
{
	$qq="SELECT a.rmId,a.sage_code,a.focus_code,a.description,b.uom AS unit_name,CONCAT(c.company_code,' - ',c.company_name) AS company_name,CONCAT(d.loc_code,' - ',d.location_name) AS location_name,a.reorder_level_qty,a.product_expiry,a.rackNo,e.category_nm ,a.location_id FROM `rm_master` AS a LEFT JOIN `uom_tbl` AS b ON a.UOM=b.ID LEFT JOIN `company_tbl` AS c ON a.company_id=c.ID LEFT JOIN `location_tbl` AS d ON a.location_id=d.ID LEFT JOIN category_master AS e ON a.catId=e.catId LEFT JOIN admin_usr_loc AS us ON a.location_id=us.locId WHERE a.sts='0' AND c.ID='$cmpId' AND us.uid='$userId' ORDER BY a.rmId LIMIT $limit";
	//echo "Else  - ".$qq."<Br>";
	
	$qqq="SELECT a.rmId,a.sage_code,a.focus_code,a.description,b.uom AS unit_name,CONCAT(c.company_code,' - ',c.company_name) AS company_name,CONCAT(d.loc_code,' - ',d.location_name) AS location_name,a.reorder_level_qty,a.product_expiry,a.rackNo,e.category_nm ,a.location_id FROM `rm_master` AS a LEFT JOIN `uom_tbl` AS b ON a.UOM=b.ID LEFT JOIN `company_tbl` AS c ON a.company_id=c.ID LEFT JOIN `location_tbl` AS d ON a.location_id=d.ID LEFT JOIN category_master AS e ON a.catId=e.catId LEFT JOIN admin_usr_loc AS us ON a.location_id=us.locId WHERE a.sts='0' AND c.ID='$cmpId' AND us.uid='$userId' ORDER BY a.rmId LIMIT $limit";
}
...
function getdata()
{
    ...
    $.ajax({
        type: 'POST',
        url: 'getrmRegister2.php',
        data:'page='+page_num+'&compId='+compId+'&iType=1'+'&locId='+locId+'&uId='+uId+'&uiType='+uType+'&role='+uRole+'&uloc='+uloc+'&srch='+srch,
        beforeSend: function () {
            //$('.loading-overlay').show();
        },
    dataContainerfunction (html) {
            $('#tbl').html(html);
        }
    });
	...
	var uRole=document.getElementById('urole').value;
	var uloc=document.getElementById('uloc').value;
 	var scriptUrl='getrmRegister2.php?compId='+compId+'&iType=1'+'&locId='+locId+'&uId='+uId+'&uiType='+uType+'&role='+uRole+'&uloc='+uloc+'&srch='+srch;
	//alert(scriptUrl);
 	//dataTable
 	$('#tbl').load(scriptUrl);*/
}
	
function searchFilter(page_num,sType)
{
    ...
    $.ajax({
        type: 'POST',
        url: 'getrmRegister2.php',
        data:'page='+page_num+'&keywords='+keywords+'&filterBy='+filterBy+'&compId='+compId+'&locId='+locId+'&uId='+uId+'&uloc='+uloc+'&uiType='+uiType
		+'&ulocty='+ulocty+'&urole='+urole,
        beforeSend: function () {
        },
