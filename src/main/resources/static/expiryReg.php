...
<head>
	...
	<script>
		...
		function getMainData() {
			...
			var url = `http://localhost:8080/manage/OFSReport/stockRegister/${companyId}/${productTypeId}/${locationId}/${productCategoryId}/${brandName}/${search}/${currentPage-1}/${pageSize}`;
			$.ajax({
				...
				success: function(data) {
					...
					data.data.forEach((data) => {
						...
						var month = Math.abs((curYear - expiryYear) * 12 + curMonth - expiryMonth);
						if (month <= 1) {
							let row = `<tr style="text-align: center; color : #f00">
							...
							let row = `<tr style="text-align: center; color : orange">
							<td>${data.entryDate}</td>
							<td>${data.location}</td>
							<td>${data.focusCode}</td>
							<td>${data.sageCode}</td>
							...
							let row = `<tr style="text-align: center; color : #0472c9">
							<td>${data.entryDate}</td>
							<td>${data.location}</td>
							<td>${data.focusCode}</td>
							<td>${data.sageCode}</td>
		...
		async function getexcel() {
			...
			try {
				const response = await fetch(`http://localhost:8080/download/OFSReport/stockRegister/${companyId}/${productTypeId}/${locationId}/${productCategoryId}/${brandName}/${search}/${currentPage-1}/${pageSize}`, {
					method: 'GET',
					headers: {
						'Authorization': token,
					},
										...
										<table cellpadding="5px">
											...
											<tr>
												...
												<td valign="top">
													...
													<input type="text" id="txtsearch" name="txtsearch" class="form-control" tabindex="3.1" placeholder="Type keywords..." onKeyUp="getSearch();" onchange="getSearch();" style="width: 240px;">
								...
								<table cellpadding="10px" cellspacing="10">
									...
									<tr>
										...
										<td valign="top">
											...
											<span style="padding-left: 10px;margin-bottom:20px;">1 Month</span>
										</td>
