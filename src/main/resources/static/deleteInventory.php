...
$qinv1="SELECT dt,compId,locId,iType,uid AS entryuId,rsts FROM inventory_tbl WHERE ID='$entryId' AND sts='0'";
...
foreach($rw as $row)
{
	...
	if($row['eType']=='1')
	{
		...
		if($rsts=='1')
		{
			$qstk1="UPDATE stock_tbl SET stk_qty='$bstkq' WHERE ID='$stkId'";
			...
			VALUES('$stkId','$compId','$locId','$iType','".$row['itemId']."','0','0','$bstkq','$stkq','$uId')";
	...
	else
	{
		...
		$qstk1="UPDATE stock_tbl SET out_qty='$boutq',stk_qty='$bstkq' WHERE ID='$stkId'";
		//echo "Del - Stock Upt : ".$qstk1."<Br><Br>";
		$ststmt1=$mysql->prepare($qstk1);
		$ststmt1->execute();
		
		$qstk2="INSERT INTO stock_upt_tbl(stkId,compId,locId,iType,itemId,outq,poutq,stkq,pstkq,uid)
		VALUES('$stkId','$compId','$locId','$iType','".$row['itemId']."','$boutq','$outq','$bstkq','$stkq','$uId')";
		//echo "Del - Stock Upt Info: ".$qstk2."<Br><Br>";
		$ststmt2=$mysql->prepare($qstk2);
		$ststmt2->execute();
	}
	...
	{
		//stocktbl2
		$qstkk="SELECT ID FROM stock_tbl_2 WHERE dt='$entrydt' AND compId='$compId' AND locId='$locId' AND iType='$iType' AND itemId='".$row['itemId']."' AND batchNo='".$row['batchNo']."' AND out_qty='".$row['qty']."' AND sts='0'";
		//echo "Del - Stock2 : ".$qstkk."<Br><Br>";
		$ststmtt=$mysql->prepare($qstkk);
		$ststmtt->execute();
		$rowstkk=$ststmtt->fetch(PDO::FETCH_ASSOC);
		...
		$qstkk12="INSERT INTO stock_tbl_2_del_tbl(stkId,stkId2,compId,locId,iType,itemId,batchNo,pbatchNo,pinq,inq,poutq,outq,plockq,lockq,uid)
		VALUES('$stkId','$stkId2','$compId','$locId','$iType','".$row['itemId']."','".$row['batchNo']."','".$row['batchNo']."','0','0','".$row['qty']."',
		'".$row['qty']."','0','0','$uId')";
