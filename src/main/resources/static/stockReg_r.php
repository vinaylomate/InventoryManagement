	...
	<script>
		...
		function getBrandName() {
			...
			$.ajax({
				...
				url: `http://localhost:8080/manage/get/productRegister/${companyId}/${productTypeId}/${productCategoryId}/null/${currentPage-1}/${pageSize}`,
				contentType: 'application/json',
				success: function(data) {
					let brandNameDD = document.getElementById('brand');
					brandNameDD.innerHTML = `<option value="0">Select</option>`;
					var arr = {};
					data.data.forEach((productRegister) => {
						arr[productRegister.brandName] = 1;
					});
					for (var key in arr) {
						let row = `<option value="${key}">${key}</option>`;
						brandNameDD.innerHTML += row;
					}
				},
				error: function(error) {
					alert(JSON.stringify(error));
		...
		function getMainData() {
			...
			var productCategoryId = document.getElementById('catId').value == "" ? 0 : document.getElementById('catId').value;
			var productTypeId = document.getElementById('iType').value == "" ? 0 : document.getElementById('iType').value;
			var brandName = document.getElementById('brand').value == "0" ? "null" : document.getElementById('brand').value;
			var inQty = 0;
			var outQty = 0;
			var lock = 0;
			var stock = 0;
			let pageSize = document.getElementById('itemsPerPageDropdown').value;
			var url = `http://localhost:8080/manage/stockReport/stockRegister/${companyId}/${productTypeId}/${locationId}/${productCategoryId}/${brandName}/${search}/${currentPage-1}/${pageSize}`;
			$.ajax({
				...
				success: function(data) {
					...
					data.data.forEach((data) => {
						let showStock = data.inQty-data.outQty;
						if (showStock < data.reorderLevel) {
		...
		async function getexcel() {
			...
			let pageSize = document.getElementById('itemsPerPageDropdown').value;
			try {
				const response = await fetch(`http://localhost:8080/download/reorder/stockRegister/${companyId}/${productTypeId}/${locationId}/${productCategoryId}/${brandName}/${search}/${currentPage-1}/${pageSize}`, {
					method: 'GET',
					headers: {
						'Authorization': token,
					},
...

		{
			...
			window.location.href = 'stockReg_r.php';
...

		{
			...
			var scriptUrl = 'mpdf/print_files/stockReg_r_pdf.php?compId=' + compId + '&iType=' + iType + '&locId=' + locId + '&uId=' + uId + '&uiType=' + uType + '&role=' + uRole + '&srch=' + srch + '&catId=' + catId + '&brand=' + brand;
			...
			<div id="content">
				...
				<div class="container-fluid">
					...
					<div class="align-items-center justify-content-between mb-2" style="background-color:#3c8dbc;border-radius:5px;" align="center">
						...
						<h1 class="h3 mb-0 text-gray-801" style="padding-bottom: 5px;">Re - Order(Stock)Report </h1>
										...
										<table cellpadding="5px">
											...
											<tr>
												...
												<td valign="top">
													...
													<select class="form-control form-control-sm" id="iType" name="iType" tabindex="2" onChange="getMainData(); getBrandName(); getLocationData(); getCategoryData();" style="width:250px;" required>
											...
											<tr>
												...
												<td valign="top">
													...
													<select class="form-control form-control-sm" id="brand" name="brand" tabindex="5" onChange="getMainData();" style="width:250px;" required>
											...
											<tr>
												...
												<td valign="top"><input type="text" id="txtsearch" name="txtsearch" class="form-control" tabindex="5.1" onKeyUp="getSearch();" onchange="getSearch();"></td>
							...
							<div align="right">
								...
								<table>
									...
									<tr>
										...
										<td><input type="button" id="btnReset" name="btnReset" class="btn btn-warning" onClick="dataReset();" tabindex="5.3" value="Reset"> </td>

										<td><a href="javascript:getexcel();"><img src="img/EXCELIMG1.png" alt="download" title="download" style="width:50px; height:50px"></a></td>

										<td><a href="javascript:getpdf();"><img src="img/pdf.png" alt="download" title="download" style="width:50px; height:50px"></a></td>
