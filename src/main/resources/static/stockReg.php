	...
	<script>
			...
			$.ajax({
				...
				success: function(data) {
					...
					data.data.forEach((data) => {
						let stockShow = data.inQty - data.outQty;
						if(stockShow < 0)stockShow = 0;
						...
						if (stockShow < data.reorderLevel) {
							row += `<td>YES</td>`
						} else {
							row += `<td>NO</td>`
						}
...

		{
			...
			$.ajax({
				...
				data: 'page=' + page_num + '&keywords=' + keywords + '&filterBy=' + filterBy + '&compId=' + compId + '&iType=' + iType + '&locId=' + locId + '&uId=' + uId + '&uiType=' + uType + '&urole=' + uRole + '&catId=' + catId + '&brand=' + brand,
	...
	//alert(scriptUrl);
 	...
 	$('#tbl').load(scriptUrl);*/
		...
		async function getexcel() {
			...
			let pageSize = document.getElementById('itemsPerPageDropdown').value;
			try {
				const response = await fetch(`http://localhost:8080/download/stockReport/stockRegister/${companyId}/${productTypeId}/${locationId}/${productCategoryId}/${brandName}/${search}/${currentPage-1}/${pageSize}`, {
					method: 'GET',
					headers: {
						'Authorization': token,
					},
										...
										<div class="p-2">
											...
											<table cellpadding="5px">
												...
												<tr>
													...
													<td valign="top"><label class="m-0 font-weight-bold text-primary">Brand Name</label></td>
													...
													<td valign="top">
														...
														<select class="form-control form-control-sm" id="brand" name="brand" tabindex="4" style="width:250px;" onChange="getMainData();">
