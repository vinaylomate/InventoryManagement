...
CONCAT(d.salemanNm) AS saleNm FROM inventory_tbl AS a LEFT JOIN company_tbl AS b ON a.compId=b.ID LEFT JOIN location_tbl AS c ON a.locId=c.ID LEFT JOIN salesman_tbl AS d ON a.salemnId=d.ID WHERE a.ID='$Id' AND a.sts='0'";
...
<h1 class="h3 mb-0 text-gray-801" style="padding-bottom: 5px;">Stock Entry Edit Form</h1>   
</div>
<form class="user" action="ins_inventoryEdit.php" method="post" id="companyMaster" name="companyMaster" enctype="multipart/form-data">		
...
<?php
include("mysql.connect.php");
$q="SELECT entryId AS ID,entryNm FROM entry_type_tbl WHERE sts='0' AND (entryNm NOT LIKE '%lock%') ORDER BY entryNm";
$s=$mysql->prepare($q);
$s->setFetchMode(PDO::FETCH_ASSOC);
$s->execute();
while($r=$s->fetch())
...
while($rw=$stmtt->fetch(PDO::FETCH_ASSOC))
{
	...
	$q11="SELECT ID,out_qty FROM stock_tbl_2 WHERE compId='$compId' AND locId='$locId' AND iType='$iTyps' AND itemId='".$rw['itemId']."' AND batchNo='".$rw['batchNo']."' AND sts='0' AND dt='$dt'";
	$stmtt11=$mysql->prepare($q11);
	$stmtt11->execute();
	$rowstk1=$stmtt11->fetch(PDO::FETCH_ASSOC);
	$stkId21=$rowstk1['ID'];
	$outQty=$rowstk1['out_qty'];	
	...
	echo '<td><label id="bar'.$k.'" style="color: black; font-weight: bold;">-</label>
	<input type="hidden" name="item_id[]" id="item_id'.$k.'" value="'.$rw['itemId'].'"></td>';	
	echo '<td><label id="desp'.$k.'" style="color: black; font-weight: bold;">'.$rw['itemNm'].'</label></td>';
	echo '<td>
	...
	<input type="text" name="batch[]" id="batch'.$k.'" style="width: 90px; font-weight: bold;" value="'.$rw['batchNo'].'">
	...
	{	
	echo '<td style="background-color: white;">
	<input type="text" name="batch[]" id="batch'.$k.'" readonly="" style="width: 90px; background-color: rgb(195, 194, 194); font-weight: bold;" value="'.$rw['batchNo'].'">
	<input type="hidden" name="pbatch[]" id="pbatch'.$k.'" style="width: 90px; font-weight: bold;" value="'.$rw['batchNo'].'"></td>';	
	}

	echo '<td><input type="text" name="stkqty[]" id="stkqty'.$k.'" readonly="" style="background-color: rgb(195, 194, 194); font-weight: bold; width: 50px;" value="'.$stkQty.'"></td>';	
	...
	echo '<td><input type="text" name="bqty[]" id="bqty'.$k.'" tabindex="10" style="background-color: rgb(195, 194, 194); font-weight: bold; width: 50px;" value="0" readonly></td>';	
	if($rw['eType']=='1')	
	{
	echo '<td style="background-color: white;">
	<input type="text" name="expdt[]" id="expdt'.$k.'" readonly="" style="width: 90px; background-color: rgb(195, 194, 194); font-weight: bold;" value="'.$rw['expdt'].'">';
	}
	else
	{
	echo '<td style="background-color: white;">
	...
	<input type="hidden" name="stkId2[]" id="stkId2'.$k.'" value="'.$stkId.'">
	<input type="hidden" name="pout_Q[]" id="pout_Q'.$k.'" value="'.$outQty.'">
	<input type="hidden" name="stkId_O2[]" id="stkId_O2'.$k.'" value="'.$stkId21.'"></td>';
	echo '</tr>';
}
$mysql=null;	
?>
...
function getproduct()	
{
  ...
  else	
  {
        ...
        var scriptUrl ='getProducts.php?iType='+id+'&compId='+compId+'&locId='+locId+'&yr='+yr;
        //alert(scriptUrl);
        $.ajax({url:scriptUrl,success:function(result)
        {	
           //alert(result);
