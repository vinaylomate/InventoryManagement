...
else
{
	if($type=='0')
	{
		$q="SELECT a.fgId AS ID,a.sage_code,a.focus_code,IF(a.barcode='','-',a.barcode) AS barcode,CONCAT(a.descc,' ',a.brand,' - ',b.uom)  AS desp,product_expiry AS expdt,reorder_level_qty AS rordQty,a.rackNo FROM fg_master AS a LEFT JOIN uom_tbl AS b ON a.uom=b.ID WHERE a.fgId='$itemId' AND a.sts='0'";
	}
	else
	{
		$q="SELECT a.fgId AS ID,a.sage_code,a.focus_code,IF(a.barcode='','-',a.barcode) AS barcode,CONCAT(a.descc,' ',a.brand,' - ',b.uom)  AS desp,product_expiry AS expdt,reorder_level_qty AS rordQty,rackNo FROM fg_master AS a LEFT JOIN uom_tbl AS b ON a.uom=b.ID WHERE a.barcode='$itemId' AND a.sts='0'";
	}
}
//echo $q."<Br>";
$s=$mysql->prepare($q);
